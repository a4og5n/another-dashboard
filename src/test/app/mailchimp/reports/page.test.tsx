...
describe("Reports Page Service Integration", () => {
    ...
    reports: [
      {
        ...
        opens: { opens_total: 350, unique_opens: 300, open_rate: 0.30, last_open: "2023-11-02T14:30:00Z" },
        clicks: { clicks_total: 120, unique_clicks: 100, unique_subscriber_clicks: 95, click_rate: 0.10, last_click: "2023-11-02T16:00:00Z" },
        facebook_likes: { recipient_likes: 25, unique_likes: 20, facebook_likes: 30 },
        industry_stats: { type: "E-commerce", open_rate: 0.25, click_rate: 0.08, bounce_rate: 0.02, unsubscribe_rate: 0.005 },
        list_stats: { sub_rate: 0.1, unsub_rate: 0.005, open_rate: 0.28, click_rate: 0.09 },
        delivery_status: { enabled: true, can_cancel: false, status: "sent", emails_sent: 1000, emails_canceled: 0 },
      },
    ],
    total_items: 1,
  };
  ...
  describe("Service Integration", () => {
    ...
    it("should handle service success response", async () => {
      ...
      const service = getMailchimpService();
      const result = await service.getCampaignReports({ count: 10 });
    ...
    it("should handle service exceptions", async () => {
      ...
      await expect(service.getCampaignReports({ count: 10 }))
  ...
  describe("Response Handling", () => {
    it("should preserve report data structure", async () => {
      ...
      const service = getMailchimpService();
      const result = await service.getCampaignReports({ count: 1 });
