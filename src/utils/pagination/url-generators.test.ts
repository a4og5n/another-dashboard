...
describe("createPaginationUrls", () => {
  ...
  describe("createPageUrl", () => {
    it("should create URL with page parameter (omits default perPage=10)", () => {
      const { createPageUrl } = createPaginationUrls("/mailchimp/reports", 10);
      const url = createPageUrl(2);
      expect(url).toBe("/mailchimp/reports?page=2");
    });

    it("should preserve current perPage setting when not default", () => {
      const { createPageUrl } = createPaginationUrls("/test", 25);
      const url = createPageUrl(3);
      expect(url).toBe("/test?page=3&perPage=25");
    });

    it("should handle page 1 (omits default page=1)", () => {
    ...
    it("should work with different perPage values", () => {
      ...
      expect(create10(2)).toBe("/test?page=2");
    ...
    it("should handle nested paths", () => {
      ...
      const url = createPageUrl(2);
      expect(url).toBe("/mailchimp/reports/123/emails?page=2");
    });

    it("should handle paths with special characters", () => {
      const { createPageUrl } = createPaginationUrls("/test-path/sub_path", 10);
      const url = createPageUrl(2);
      expect(url).toBe("/test-path/sub_path?page=2");
    });
  });

  describe("createPerPageUrl", () => {
    it("should create URL with new perPage and reset to page 1 (omits defaults)", () => {
      const { createPerPageUrl } = createPaginationUrls("/test", 10);
      const url = createPerPageUrl(25);
      expect(url).toBe("/test?perPage=25");
    });
    ...
    it("should always reset to page 1 regardless of current page", () => {
      ...
      expect(url1).toBe("/test?perPage=50");
      expect(url2).toBe("/test?perPage=100");
    });

    it("should handle different perPage options (omits defaults)", () => {
      const { createPerPageUrl } = createPaginationUrls("/test", 10);

      expect(createPerPageUrl(10)).toBe("/test");
      expect(createPerPageUrl(25)).toBe("/test?perPage=25");
      expect(createPerPageUrl(50)).toBe("/test?perPage=50");
      expect(createPerPageUrl(100)).toBe("/test?perPage=100");
    });
    ...
    it("should work with nested paths", () => {
      ...
      expect(url).toBe("/mailchimp/lists/456/activity?perPage=50");
    });
  });
  ...
  describe("Real-World Scenarios", () => {
    it("should support email activity table pagination", () => {
      ...
      expect(createPageUrl(3)).toBe(
        "/mailchimp/reports/abc123/email-activity?page=3",
      );

      // Change to 50 items per page (resets to page 1)
      expect(createPerPageUrl(50)).toBe(
        "/mailchimp/reports/abc123/email-activity?perPage=50",
      );
    });
    ...
    it("should support locations table pagination", () => {
      ...
      // Navigate through pages (perPage=25 is not default, so it's included)
      expect(createPageUrl(1)).toBe(
        "/mailchimp/reports/xyz789/locations?perPage=25",
      );
      ...
      expect(createPerPageUrl(100)).toBe(
        "/mailchimp/reports/xyz789/locations?perPage=100",
      );
    });
    ...
    it("should support list activity table pagination", () => {
      ...
      expect(createPageUrl(5)).toBe("/mailchimp/lists/list123/activity?page=5");
      expect(createPerPageUrl(25)).toBe(
        "/mailchimp/lists/list123/activity?perPage=25",
      );
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle very large perPage values", () => {
      ...
      expect(createPageUrl(1)).toBe("/test?perPage=1000");
      expect(createPerPageUrl(10000)).toBe("/test?perPage=10000");
    });
    ...
    it("should handle base URL with existing query parameters (overwrites them)", () => {
      ...
      expect(url).toBe("/test?foo=bar?page=2");
  ...
  describe("Immutability", () => {
    ...
    it("should create independent URL generators", () => {
      ...
      expect(gen1.createPageUrl(2)).toBe("/test1?page=2");
